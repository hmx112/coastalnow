...
def _home(groups):
 all_items=[x for v in groups.values() for x in v]; data=json.dumps([{'name':x['name'],'state':x['state'],'url':x['page_path'],'status':location_status(x)} for x in all_items],ensure_ascii=False)
 ...
 body+=f'''<div class="preview-note"><strong>Data status:</strong> San Diego and Los Angeles are Live NOAA pages. All other locations are Preview pages.</div><section class="section" id="search"><div class="section-head"><h2>Find a beach or city</h2><p>Search 51 coastal locations</p></div><div class="search-box"><input id="q" placeholder="San Diego, Florida, Holden Beach…"><button onclick="go()">Search</button></div><div id="results" class="directory-grid"></div></section><section class="section" id="states"><div class="section-head"><h2>Browse by state</h2><p>State directories</p></div><div class="directory-grid">{"".join(_state_card(k,v) for k,v in groups.items())}</div></section><section class="section"><div class="section-head"><h2>All coastal locations</h2><p>Live NOAA and Preview status</p></div><div class="directory-grid">{"".join(_card(x,x["page_path"]) for x in all_items)}</div></section><div class="ad-slot"><span>ADVERTISEMENT</span></div><script>const L={data};function go(){{const q=document.getElementById('q').value.toLowerCase().trim();document.getElementById('results').innerHTML=q?L.filter(x=>(x.name+' '+x.state).toLowerCase().includes(q)).slice(0,12).map(x=>`<a class="info-card" href="${{x.url}}"><span class="status-badge ${{x.status==='Live NOAA'?'badge-live':'badge-preview'}}">${{x.status}}</span><h3>${{x.name}}</h3><p>${{x.state}}</p></a>`).join('')||'<p>No result</p>':''}}</script>'''
 return _shell('CoastalNow — Tide Times by U.S. Coastal Location','Browse tide times by U.S. coastal location.','',body)
def _state_page(items):
 name=items[0]['state']; cards=''.join(_card(x,x['slug']+'/index.html') for x in items)
 body=f'<div class="breadcrumbs"><a href="../../index.html">Home</a><span>/</span>{escape(name)}</div>'+_hero(f'{items[0]["state_code"]} DIRECTORY',f'{name} Tide Times',f'Browse {len(items)} coastal locations with the same clear status and page layout.')+f'<div class="preview-note"><strong>Data status:</strong> Live NOAA pages are marked clearly; all other locations remain Preview pages.</div><section class="section"><div class="section-head"><h2>Choose a coastal location</h2><p>{len(items)} locations</p></div><div class="directory-grid">{cards}</div></section><div class="ad-slot"><span>ADVERTISEMENT</span></div>'
 return _shell(f'{name} Tide Times | CoastalNow',f'Browse tide pages for coastal locations in {name}.','../../',body)
